...
public class PubMan extends StdGame {
	public static void main(String [] args) {new PubMan(parseSizeArgs(args));}
	public PubMan() { initEngineApplet(19,15,16,16,null,null,null); }
	public PubMan(Dimension size) {
		initEngine(19,15,16,16,null,null,null,size.width,size.height); 
	}
	public void initGame() {
		defineMedia("examples/pub_man.tbl");
		setFrameRate(40,4);
	...
	int nr_pills;
	int powerpilltime=0,powerpillmax, powerpillcaught;
	public void doFrameInGame() {
		...
		if (powerpilltime > 0) powerpilltime--;
	}
	public void paintFrame() {
		drawImageString(""+score,0,0,-1,"font_map",32,2);
		for (int i=1; i<lives; i++) drawImage(pfWidth()-16*i,0,"player_d7");
	}
	...
	public void startInGame() {
		...
		ghost_house = new JGObject("ghost_house",false,9*pfTileWidth(),
			5*pfTileHeight()-2,8,null);
			...
			for (int px=1; px<pfTilesX()/2-1; px++) {
				...
				if (and(getTileCid(px,py),2)) {
					new JGObject("powerpill",true,
						px*pfTileWidth(),py*pfTileHeight(),4,"powerpill");
					new JGObject("powerpill",true,
						(pfTilesX()-1-px)*pfTileWidth(),py*pfTileHeight(),4,
						"powerpill");
					break;
				}
			}
		...
		public void move() {
			if (is_eaten) { // return to ghost house
				...
				if (powerpilltime > 0 && !is_restored) { // flee
					home_in=player;
					avoid=true;
					if (powerpilltime<80 && (powerpilltime&8)!=0) {
						gfx_prefix="enemy_w_";
					} else {
						gfx_prefix = "enemy_b_";
					}
			...
			if (!(obj instanceof Player)) { // ghost house
				...
				if (powerpilltime > 0 && !is_eaten && !is_restored) {
					...
					new StdScoring("Scoring",x+8,y+4,0,0, 80,
						...
						new Color[] {Color.white,Color.green}, 16);
					powerpillcaught++;
				}
	...
	Font scoring_font = new Font("Helvetica",0,32);
	public class Scoring extends JGObject {
		String msg;
		public Scoring(String message,double x,double y) {
			super("scoring",true,x+8,y,0,null,0,0,80);
			msg=message;
		}
		public void paint() {
