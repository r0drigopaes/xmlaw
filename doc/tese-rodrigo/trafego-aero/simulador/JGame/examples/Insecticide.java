...
public class Insecticide extends JGEngine {
	...
	int lives=3;
	int level=0,stage=0;
	Color bg_color = new Color(255,240,0);
	Color fg_color = new Color(100,50,0);
	public static void main(String [] args) { new Insecticide(0,0); }
	public Insecticide(int w,int h){
		initEngine(20,15,32,32,fg_color,bg_color,null,w,h);
	}
	public Insecticide() {initEngineApplet(20,15,32,32,fg_color,bg_color,null);}

	public void initGame() {
		defineMedia("examples/insecticide.tbl");
		setFrameRate(40,4);
		setGameState("Title");
		createBackground();
		setMsgFont(new Font("Helvetica",0,32));
	}
	...
	public void doFrameInGame() {
		...
		&& existsObject("player")) {
			int xpos = (getObject("player").x < pfWidth()/2) ?
					pfWidth() : -pfTileWidth();
			new Spider(xpos, random(pfHeight()/2,pfHeight()-pfTileHeight()),
		...
		&& existsObject("player")) {
			...
			int xpos = xdir < 0 ? pfWidth() : -pfTileWidth();
			new Beetle(xpos, random(0,pfHeight()-3*pfTileHeight()),
		...
		&& existsObject("player")) {
			int xdir = (getObject("player").x < pfWidth()/2) ? -1 : 1;
			int xpos = xdir < 0 ? pfWidth() : -pfTileWidth();
			new Worm(xpos, random(0,pfHeight()-2*pfTileHeight()),xdir*4.0);
		}
		moveObjects();
		checkCollision(2,1);
		checkCollision(1,4);
	...
	public class Player extends JGObject {
		public Player () { 
			super("player",false,pfWidth()/2,pfHeight()-pfTileWidth(),4,"ship");
		}
		public void move() {
	...
	public class Spider extends JGObject {
		...
		public void hit_bg(int tilecid) {
			...
			Rectangle rlast = getLastTileBBox();
			Rectangle rthis = getTileBBox();
			if (rlast==null || rthis==null) return;
			Rectangle r = new Rectangle(rthis);
			r.x = rlast.x;
	...
	public class Beetle extends JGObject {
		...
		public void hit_bg(int tilecid,int tx,int ty) {
			setTile(tx,ty,"");
			setPos(tx*pfTileWidth(),ty*pfTileHeight());
		}
	}
