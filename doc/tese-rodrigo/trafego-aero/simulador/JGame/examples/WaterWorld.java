...
public class WaterWorld extends StdGame {
	public static void main(String [] args) {
		new WaterWorld(parseSizeArgs(args));
	}
	public WaterWorld() { initEngineApplet(20,15,32,32,null,null,null); }
	public WaterWorld(Dimension size){
		initEngine(20,15,32,32,null,null,null, size.width,size.height);
	}

	public String action_msg=null;
	public Player player;
	public int level_nr; // determines platforms and monster freqencies
	public int gamespeed; // determines certain speed and regeneration factors
	...
	public void initGame() {
		defineMedia("examples/water_world.tbl");
		setFrameRate(40,4);
		startgame_ticks=60;
		lifelost_ticks=100;
		leveldone_ticks=100;
	...
	public void doFrame() {
		...
		if (timer%(lev.ball_freq-3*gamespeed) == 0) {
			new Ball(random(-pfTileHeight(),pfWidth()-32),
				-pfTileHeight(),
				-1 + 2*(int)(Math.random()+0.5) );
		}
		if (timer%(lev.bird_freq-3*gamespeed) == 0) {
			int x,dir,y;
			while (true) {
				...
				y=(int)random(32,(pfTilesY()-lev.waterlevel)*pfTileHeight(),64);
				int cid = getTileCidAtCoord(x+pfTileWidth()*dir,y);
				if ( (cid&1+2+8) == 0) break;
			}
			new Bird(x,y,dir);
		}
		if (timer%(lev.fish_freq-2*gamespeed) == 0) {
			int x,dir,y;
			int timeout=8;
			while ((timeout--) > 0) {
				if (random(0,2) >= 1.0) {
	...
	public class Player extends JGObject {
		...
		public void move() {
			if (!swimming && (checkBGCollision(getTileBBox())&8)!=0) {
				...
				jumptime=0;
				setAnim("player_swd");
				falltime = (int)(pfTileHeight()/speed);
			}
			if (!swimming) {
				moveNorm();
			} else {
	...
	public class Ball extends JGObject {
		double speed=1.7 + 0.3*gamespeed;
		double dir=1;
		...
		public void hit_bg(int tilecid,int tx,int ty,int txsize,int tysize) {
			...
			if ((tilecid&8) != 0) {
				...
				if (((tlcid|trcid)&8)!=0) { /* we're below the surface */
					...
					if (y > 0 && (int)y % pfTileHeight() > pfTileHeight()*0.7) {
						y--;
					} else {
						y+=2;
					}
	...
	public class Crab extends JGObject {
		double speed=1.5 + 0.2*gamespeed;
		double dir=1;
		char orient='u';
		...
		public void move() {
			...
			if (!isOnScreen((int)-speed,(int)-speed)) remove(); 
		}
		public void hit(JGObject obj) {
			remove();
			obj.remove();
	...
	public class Fish extends JGObject {
		...
		double ydir=0;
		boolean was_falling=false;
		int crabtimer=24, crabmax=32-2*gamespeed;
