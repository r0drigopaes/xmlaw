...
{
    /**
     * JSIM Bean Box execution directory (FIX -- must be .../beanbox)
     */
    static final String JAR_DIR = System.getProperty ("user.dir");
    ...
    ToolBox (int x, int y)
    {
        ...
        setLayout (null);
        setForeground (BeanBoxFrame.FG_COLOR);
        setBackground (BeanBoxFrame.BG_COLOR);
...
class ToolBoxPanel extends Panel implements Runnable, MouseListener
{
    ...
    public void paint (Graphics g)
    {
        ...
        for (int i = 0; i < beanLabels.size (); i++) {
            ...
            g.drawString
                (name, sidePad + 21, topPad + (rowHeight*i) + rowHeight-4);
            Image img = (Image)beanIcons.elementAt(i);
            if (img != null) {
                boolean status = g.drawImage
                    (img, sidePad+2, topPad + (rowHeight*i) + 2, 16, 16, this);
            }; // if
        }; // for
    ...
    /*************************************************************
     * Provide information on all the loaded JAR files, regardless
     * of origin.
     */
    ...
    /*************************************************************
     * Get the names of all jar files in JAR_DIR.
     */
    private static Vector getJarNames ()
    {
        //File cwd = new File (System.getProperty ("user.dir"));
        File cwd  = new File (ToolBox.JAR_DIR);
        File pwd  = new File (cwd.getParent ());
        File jars = new File (pwd, "jars");

        if (! jars.isDirectory ()) {
            System.err.println ("getJarNames: " + jars +
                                " is not a directory!!");
        }; // if

        Vector result   = new Vector ();
        String [] names = jars.list (new FileExtension (".jar"));
    ...
    public void mousePressed (MouseEvent evt)
    {
        ...
        if (beanName.equals("jsim.beanbox.BeanBox")) {
            bean = new BeanBox();
        } else {
