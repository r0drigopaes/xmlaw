...
import br.pucrio.inf.les.law.parser.XMLawReader;
import br.pucrio.inf.les.law.parser.handler.base.ComposedHandler;

public class IncreaseHandler extends ComposedHandler{

	private static final Log LOG = LogFactory.getLog(IncreaseHandler.class);
	
	public IncreaseHandler(Stack<Object> stack, DescriptorTable table, Hashtable<Id, Object> handlerCache) {
	...
	public void process(String namespaceURI, String localName, String qName, Attributes attr) throws LawException {
		...
		EventDescriptor increase = new EventDescriptor(id,eventId,eventType,value);
		CriticalityAnalysisDescriptor criticalityAnalysisDescriptor = (CriticalityAnalysisDescriptor) stack.peek(); //CriticalityAnalysis
		criticalityAnalysisDescriptor.addIncreaseDescriptors(increase);
		table.add(increase);
		stack.push(increase);
	}
